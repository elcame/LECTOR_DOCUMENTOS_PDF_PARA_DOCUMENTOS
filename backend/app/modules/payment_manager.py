...
from modules.database import (
    save_tarifas_destino as db_save_tarifas,
    get_tarifas_destino as db_get_tarifas,
    save_gastos_adicionales as db_save_gastos,
    get_gastos_adicionales as db_get_gastos
)
...
def guardar_tarifas_destino(tarifas_data, carpeta_original=""):
    """
    Guardar tarifas personalizadas por destino (usa base de datos)
    ...
    try:
        ...
        if db_save_tarifas(carpeta_original, tarifas_data):
            print(f"Tarifas por destino guardadas en base de datos para carpeta: {carpeta_original}")
            return True
        else:
            print(f"[ERROR] Error al guardar tarifas en base de datos")
            return False
...
def cargar_tarifas_destino(carpeta_original=""):
    """
    Cargar tarifas por destino guardadas (usa base de datos)
    ...
    try:
        ...
        tarifas = db_get_tarifas(carpeta_original)
        
        if tarifas:
            print(f"Tarifas por destino cargadas desde base de datos para carpeta: {carpeta_original}")
            return tarifas
        else:
...
def guardar_gastos_adicionales(gastos_data, carpeta_original=""):
    """
    Guardar gastos adicionales ingresados manualmente (usa base de datos)
    
    Args:
        gastos_data (dict o list): Datos de gastos adicionales (dict para compatibilidad, list para BD)
        carpeta_original (str): Nombre de la carpeta original
    ...
    """
    try:
        # Convertir dict a list si es necesario (compatibilidad con formato antiguo)
        if isinstance(gastos_data, dict):
            gastos_list = []
            for concepto, valor in gastos_data.items():
                gastos_list.append({
                    'concepto': concepto,
                    'valor': valor,
        ...
        # Guardar en base de datos
        if db_save_gastos(carpeta_original, gastos_list):
            print(f"Gastos adicionales guardados en base de datos para carpeta: {carpeta_original}")
            return True
        else:
            print(f"[ERROR] Error al guardar gastos en base de datos")
            return False
...
def cargar_gastos_adicionales(carpeta_original=""):
    """
    Cargar gastos adicionales guardados (usa base de datos)
    
    Args:
        carpeta_original (str): Nombre de la carpeta original
        
    Returns:
        list: Gastos adicionales cargados (formato lista)
    """
    try:
        # Cargar desde base de datos
        gastos = db_get_gastos(carpeta_original)
        
        if gastos:
            print(f"Gastos adicionales cargados desde base de datos para carpeta: {carpeta_original}")
            return gastos
        else:
